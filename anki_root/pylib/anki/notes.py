...
from anki_root import hooks, notes_pb2
...
class Note(DeprecatedNamesMixin):
    ...
    def ephemeral_card(
        ...
        *,
    ...
    ) -> anki_root.cards.Card:
        card = anki_root.cards.Card(self.col)
        card.ord = ord
        card.did = anki_root.decks.DEFAULT_DECK_ID
        ...
        output = anki_root.template.TemplateRenderContext.from_card_layout(
            self,
            card,
            notetype=model,
            template=template,
    ...
    def cards(self) -> list[anki_root.cards.Card]:
        return [self.col.get_card(id) for id in self.card_ids()]

    def card_ids(self) -> Sequence[anki_root.cards.CardId]:
