...
from anki_root import cards_pb2, hooks
...
class Card(DeprecatedNamesMixin):
    ...
    lastIvl: int
    ord: int
    nid: anki_root.notes.NoteId
    id: CardId
    did: anki_root.decks.DeckId
    odid: anki_root.decks.DeckId
    queue: CardQueue
    type: CardType
    ...
    def __init__(
        self,
        col: anki_root.collection.Collection,
        id: CardId | None = None,
        backend_card: BackendCard | None = None,
    ) -> None:
        self.col = col.weakref()
        self.timer_started: float | None = None
        self._render_output: anki_root.template.TemplateRenderOutput | None = None
        if id:
            # existing card
            self.id = id
            self.load()
    ...
    def _load_from_backend_card(self, card: cards_pb2.Card) -> None:
        ...
        self.nid = anki_root.notes.NoteId(card.note_id)
        self.did = anki_root.decks.DeckId(card.deck_id)
        ...
        self.odid = anki_root.decks.DeckId(card.original_deck_id)
    ...
    ) -> anki_root.template.TemplateRenderOutput:
        if not self._render_output or reload:
            self._render_output = (
                anki_root.template.TemplateRenderContext.from_existing_card(
                    self, browser
                ).render()
            )
        return self._render_output

    def set_render_output(self, output: anki_root.template.TemplateRenderOutput) -> None:
    ...
    def current_deck_id(self) -> anki_root.decks.DeckId:
        return anki_root.decks.DeckId(self.odid or self.did)
