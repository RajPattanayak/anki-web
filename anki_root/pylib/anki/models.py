...
class ModelManager(DeprecatedNamesMixin):
    ...
    def nids(self, ntid: NotetypeId) -> list[anki_root.notes.NoteId]:
        "Note ids for M."
...
and notes.mid = ? and cards.ord = ?""",
    ...
    def get_single_notetype_of_notes(
        self, note_ids: Sequence[anki_root.notes.NoteId]
    ) -> NotetypeId:
        return NotetypeId(
    ...
    def change(
        self,
        notetype: NotetypeDict,
        nids: list[anki_root.notes.NoteId],
    ...
    def _availClozeOrds(
        ...
        import anki_root.notes_pb2

        note = anki_root.notes_pb2.Note(fields=[flds])
