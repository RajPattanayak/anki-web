...
import anki_root.buildinfo
from anki_root import _rsbridge, backend_pb2, i18n_pb2
from anki_root._backend_generated import RustBackendGenerated
from anki_root._fluent import GeneratedTranslations
...
if _rsbridge.buildhash() != anki_root.buildinfo.buildhash:
    raise Exception(
        f"""rsbridge and anki build hashes do not match:
{_rsbridge.buildhash()} vs {anki_root.buildinfo.buildhash}"""
    )
...
class RustBackend(RustBackendGenerated):
    ...
    def __init__(
        ...
        if langs is None:
            langs = [anki_root.lang.current_lang]
