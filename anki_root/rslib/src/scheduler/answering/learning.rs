...
impl CardStateUpdater {
    pub(super) fn apply_new_state(
        ...
        println!(
            "[DEBUG] Applying NEW state -> Due: {}, Queue: {:?}",
            next.position,
            CardQueue::New
        );
    ...
    pub(super) fn apply_learning_state(
        ...
        match interval {
            IntervalKind::InSecs(secs) => {
                let due = self.fuzzed_next_learning_timestamp(secs);
                println!(
                    "[DEBUG] Learning (InSecs): Scheduled after {} seconds -> Due: {}",
                    secs, due
                );
                self.card.queue = CardQueue::Learn;
                self.card.due = due;
            ...
            IntervalKind::InDays(days) => {
                let due = (self.timing.days_elapsed + days) as i32;
                println!(
                    "[DEBUG] Learning (InDays): Scheduled after {} days -> Due: {}",
                    days, due
                );
                self.card.queue = CardQueue::DayLearn;
                self.card.due = due;
    ...
    /// Adds secs + fuzz to current time
    pub(super) fn fuzzed_next_learning_timestamp(&self, secs: u32) -> i32 {
        let final_secs = self.learning_ivl_with_fuzz(self.fuzz_seed, secs);
        let now = TimestampSecs::now().0 as i32;
        println!(
            "[DEBUG] Fuzzed timestamp: now = {}, interval = {}, result = {}",
            now, final_secs, now + final_secs as i32
        );
        now + final_secs as i32
    }
    ...
    pub(super) fn learning_ivl_with_fuzz(&self, input_seed: Option<u64>, secs: u32) -> u32 {
        if let Some(seed) = input_seed {
            ...
            let upper_exclusive = secs + ((secs as f32) * 0.25).min(300.0).floor() as u32;
            if secs >= upper_exclusive {
                println!("[DEBUG] No fuzz applied. Interval: {}", secs);
                secs
            } else {
                let result = rng.random_range(secs..upper_exclusive);
                println!(
                    "[DEBUG] Fuzz applied. Range: {}..{}, Result: {}",
                    secs, upper_exclusive, result
                );
                result
            }
        } else {
            println!("[DEBUG] No fuzz seed, using raw secs: {}", secs);
            secs
        }
    }
}
