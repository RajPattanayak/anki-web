...
impl Card {
    ...
    fn set_due_date(
        ...
        debug!(
            "Setting due date for card {:?}: today={}, next_day_start={}, days_from_today={}, ease_factor={}, force_reset={}, fsrs_enabled={}",
            self.id, today, next_day_start, days_from_today, ease_factor, force_reset, fsrs_enabled
        );
        ...
        let new_interval = if fsrs_enabled {
            ...
            debug!("Resetting interval due to force_reset or non-review card.");
            days_from_today.max(1)
        } else {
            debug!("Retaining existing interval: {}", self.interval);
            self.interval.max(1)
        };

        debug!(
            "Computed new interval: {}, new due: {}, converted ease_factor: {}",
            new_interval,
            new_due,
            (ease_factor * 1000.0).round() as u16
        );
    ...
    fn schedule_as_review(&mut self, interval: u32, due: i32, ease_factor: u16) {
        debug!(
            "Scheduling as review -> interval: {}, due: {}, ease_factor: {}, card_id: {:?}",
            interval, due, ease_factor, self.id
        );
